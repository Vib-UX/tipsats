...
export async function createBoltzSwap(
  ...
  try {
    ...
    // Enter the USDT amount in the receive field (2nd amount input)
    log(`Setting receive amount: ${usdtAmount} USDT...`);
    const receiveInput = page.locator('input[placeholder="0"]').nth(1);
    await receiveInput.click();
    await receiveInput.fill(String(usdtAmount));
    await page.waitForTimeout(2000);

    // Read the calculated sats amount from the send field
    const sendInput = page.locator('input[placeholder="0"]').first();
    const satsAmount = await sendInput.inputValue();
    log(`  Sats required: ~${satsAmount}`);
    ...
    // Read sats amount from the "Pay this invoice about X sats" heading
    let finalSats = satsAmount;
    try {
    ...
    if (!bolt11 || !bolt11.startsWith("lnbc")) {
      ...
      if (lnMatch) {
        ...
        return { swapId, bolt11: lnMatch[1], satsAmount: finalSats };
      }
      throw new Error(
    ...
    return { swapId, bolt11, satsAmount: finalSats };
  } finally {
